    ...
    <div>
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            <div class="block mb-8 mb-4">
                ...
                <a href="{{ route('employees.create', $company_id) }}" class="bg-gray-200 hover:bg-gray-300 text-black  py-2 px-4 rounded">Cadastrar Funcionário</a>
                ...
                <div class="relative">
                    <form method="GET" action="{{ route('employees.index', $company_id) }}">
                        ...
                        <input name="query-employees" value="{{ $busca }}" type="search" id="default-search" class="mb-2 block w-full p-4 pl-10 text-sm text-black border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:border-gray-50 dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500 text-gray-500 bg-gray-50" placeholder="Pesquisar">
                                            ...
                                            @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                                                <button class="flex text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300 transition">
                                                    @if (Employee::findOrFail($employee->id)->employee_photo_path)
                                                        <img class="h-8 w-8 rounded-full object-cover" src="/storage/{{ Employee::findOrFail($employee->id)->employee_photo_path }}" alt="{{ Employee::findOrFail($employee->id)->name }}" />
                                                    @else
                                                        <img class="h-8 w-8 rounded-full object-cover" src="{{ Employee::findOrFail($employee->id)->profile_photo_url }}" alt="{{ Employee::findOrFail($employee->id)->name }}" />
                                                    @endif    
                                                </button>
                                            @endif
                                        </td>
